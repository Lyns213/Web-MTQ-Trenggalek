...
class NilaiRemajaController extends Controller
{
    public function index($id = null)
    {
        ...
        $records = NilaiRemaja::with('peserta')
            ->join('pesertas', 'nilai_remajas.peserta_id', '=', 'pesertas.id')
            ->where('pesertas.tahun_id', $tahunId)
            ->orderBy('pesertas.no_peserta')
            ->select('nilai_remajas.*')
            ->get();
        ...
        if (!$id) {
            ...
            if (!$currentRecord) {
                return view('filament.timerremaja', [
                    'currentRecord' => null,
                    'nextRecord' => null,
                    'previousRecord' => null,
                    'records' => $records,
            ...
            $currentRecord = NilaiRemaja::with('peserta')
                ->join('pesertas', 'nilai_remajas.peserta_id', '=', 'pesertas.id')
                ->where('nilai_remajas.id', $id)
                ->where('pesertas.tahun_id', $tahunId)
                ->select('nilai_remajas.*')
                ->first();
        ...
        $nextRecord = NilaiRemaja::with('peserta')
            ->join('pesertas', 'nilai_remajas.peserta_id', '=', 'pesertas.id')
            ->where('pesertas.tahun_id', $tahunId)
        ...
        $previousRecord = NilaiRemaja::with('peserta')
            ->join('pesertas', 'nilai_remajas.peserta_id', '=', 'pesertas.id')
            ->where('pesertas.tahun_id', $tahunId)
            ->where('pesertas.no_peserta', '<', $currentNoPeserta)
        ...
        return view('filament.timerremaja', compact('currentRecord', 'nextRecord', 'previousRecord', 'records', 'timer', 'timerInSeconds'));
    }
}
